<template>
  <div>
    ...
    <nav-bread>
      <span slot="bread" v-if="this.page === 'Address'">确认地址</span>
      <span slot="bread" v-if="this.page === 'CheckOrder'">查看订单</span>
      <span slot="bread" v-if="this.page === 'Payment'">付款</span>
      <span slot="bread" v-if="this.page === 'OrderConfirmation'">订单确认</span>
    </nav-bread>
    <div class="content_bread">
      <ul>
        <li class="Address_bread" :class="{'check': true}">确认地址</li>
        <li class="Address_bread" :class="{'check': this.page === 'CheckOrder' || this.page === 'Payment' || this.page === 'OrderConfirmation'}">查看订单</li>
        <li class="Address_bread" :class="{'check': this.page === 'Payment' || this.page === 'OrderConfirmation'}">付款</li>
        <li class="Address_bread" :class="{'check': this.page === 'OrderConfirmation'}">订单确认</li>
      </ul>
    </div>
    <Address v-if="this.page === 'Address'"></Address>
    <Order v-if="this.page === 'CheckOrder'"></Order>
    <Payment v-if="this.page === 'Payment'"></Payment>
    <OrderConfirmation v-if="this.page === 'OrderConfirmation'"></OrderConfirmation>
...
export default {
  ...
  components: {
    ...
    Modal,
    // eslint-disable-next-line vue/no-unused-components
    Address: () => import(/* webpackChunkName: "Address" */ './../components/Address.vue'),
    // eslint-disable-next-line vue/no-unused-components
    Order: () => import(/* webpackChunkName: "Order" */ './../components/Order.vue'),
    // eslint-disable-next-line vue/no-unused-components
    Payment: () => import(/* webpackChunkName: "Payment" */ './../components/Payment.vue'),
    // eslint-disable-next-line vue/no-unused-components
    OrderConfirmation: () => import(/* webpackChunkName: "OrderConfirmation" */ './../components/OrderConfirmation.vue')
  },
  watch: {
    $route: {
      immediate: true,
      handler (val, oldval) {
        if (!val.fullPath.match(/&/)) {
          this.page = 'Address'
          document.title = '确认地址'
