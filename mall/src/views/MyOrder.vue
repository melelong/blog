<template>
  <div>
    ...
    <nav-bread>
      <span slot="bread">我的订单</span>
    </nav-bread>
    <div class="content">
      <div class="content_bread">
      <ul>
        <li class="Address_bread check">我的订单</li>
      </ul>
    </div>
        <div class="abcd" v-for="(item, index) in this.AllOrderdata" :key="index">
          <div class="cart">
        <div class="page-title-normal">
          <h2 class="page-title-h2 " style="text-align: center;">
            <span>{{item.createDate}}</span>
            <img  v-lazy="require(`./../../static/img/${item.orderStatus ? '已完成.png' : '未完成.png'}`)" alt="">
          </h2>
        </div>
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <li v-for="(item1, index) in item.goodsList" :key="index">
                <div class="cart-tab-1">
                  <div class="cart-item-pic">
                    <img v-lazy="require(`./../../static/img/${item1.productImage}`)">
                  </div>
                  <div class="cart-item-title">
                    <div class="item-name">{{item1.productName}}</div>
                  </div>
                </div>
                <div class="cart-tab-2">
                  <div class="item-price">{{item1.salePrice}}</div>
                </div>
                <div class="cart-tab-3">
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <span class="select-ipt">{{'×'+item1.productNum}}</span>
                      </div>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{(item1.salePrice*item1.productNum)+'元'}}</div>
                </div>
              </li>
            </ul>
          </div>
          ...
          <div class="order">
            <ul>
              ...
              <li class="order-total">
                <span>收货电话:</span>
                <span>{{item.addressInfo.tel}}</span>
              </li>
              <li class="order-total">
                <span>收货地址:</span>
                <span>{{item.addressInfo.streetName}}</span>
              </li>
              <li class="order-total">
                <span>运输方式:</span>
              ...
              <li class="order-total total">
                <span>合计:</span>
                <span>{{item.orderTotal + '元'}}</span>
              </li>
            </ul>
          </div>
          <a href="javascript:;" @click="mdshowdel = true; ChooseOrder = item.orderId">
            <div class="cancel">
              {{item.orderStatus ? '删除' : '取消'}}
            </div>
          </a>
        </div>
    </div>
    <modal :mdshow="mdshowdel"  @close="closeModal">
      <p slot="message">你确认要删除此订单吗?</p>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="delOrder">确定</a>
...
export default {
  ...
  // created () {
  //   console.log('创建MyOrder')
  // },
  // destroyed () {
  //   console.log('销毁MyOrder')
  // },
  components: {
    NavHeader,
    NavFooter,
  ...
  methods: {
    ...
    // 获取订单请求
    async getAllOrder () {
      const getAllOrderdata = await this.Private.reqOrderList()
      const res = getAllOrderdata.data
      if (res) {
        if (res.status === '0') {
          this.AllOrderdata = res.result
        } else {
          console.log(res.msg)
        }
      } else {
